...
function solution(input) {
  ...
  function dfs(start, end) { // 문자열의 처음과 끝을 인수로 재귀함수 제작.
    if (start > end) return; // 처음에는 문제 없으나 재귀 과정에서, 오른쪽이 비면 재귀를 빠져나가야 함.
      
    let minChar = '{'; // 모든 문자 중 Z가 가장 크므로, 기준 최소 갱신을 위해 사용.
    let minIdx = -1; // 배열 내 인덱스랑 안 겹치게 -1 사용.

    for (let i = start; i <= end; i++) { // start와 end로 잡은 곳을 기준으로 문자열 배열 순회 시작.
      if (!visited[i] && str[i] < minChar) { // 방문 안했고, 현재 minChar보다 작으면 갱신.
    ...
    if (minIdx === -1) return; // 근데 만약, minIdx가 그대로라면 전부 순회한 것이므로 돌아감.
    visited[minIdx] = true; // 다시 사용하면 안되므로 방문 지정.
    output[minIdx] = str[minIdx]; // 가장 작은 것을 str에서 따와 그대로 삽입. (후에 퍼즐 마냥 완성되는 방식!)
    console.log(output.join('')); // 배열이었으므로, 문자열화해서 출력.
      
    // 출력했으니 다음 작은 녀석을 찾아야 함. 로직은 위에 이미 완성했으므로 방향만 잡아주면 됨.
    dfs(minIdx + 1, end); // 우측일 때는 minIdx의 '다음 것부터' 끝에 것까지 비교해야 함.
    dfs(start, minIdx - 1); // 좌측일 때는 시작에서부터 minIdx의 '전 것까지' 비교해야함.
  }
   
  dfs(0, str.length - 1); // 실질적인 시작.
}